...
describe('TaskController', () => {
  ...
  beforeEach(async () => {
    ...
    findAllTasksByTitleUseCase = module.get<FindAllTasksByTitleUseCase>(FindAllTasksByTitleUseCase);
    findTagByIdUseCase = module.get<FindTagByIdUseCase>(FindTagByIdUseCase);
    findAllTasksByTagUseCase = module.get<FindAllTasksByTagUseCase>(FindAllTasksByTagUseCase);
  });
  ...
  describe('findAll', () => {
    it('should return an array of tasks', async () => {
      const tasks = [
        new Task({ id: '1', title: 'Task 1', description: 'Task 1 description', dateTime: now, duration: 10 }),
        new Task({ id: '2', title: 'Task 2', description: 'Task 2 description', dateTime: now, duration: 10 })
      ];
  ...
  describe('findById', () => {
    it('should return the task with the specified ID', async () => {
      const taskId = '1';
      const task = new Task({ id: taskId, title: 'Task 1', description: 'Task 1 description', dateTime: now, duration: 10 })
  ...
  describe('findByTitle', () => {
    it('should return an array of tasks with the specified title', async () => {
      ...
      const tasks = [
        new Task({ id: '1', title: title, description: 'Task 1 description', dateTime: now, duration: 10 }),
        new Task({ id: '2', title: title, description: 'Task 2 description', dateTime: now, duration: 10 })
      ];
      jest.spyOn(findAllTasksByTitleUseCase, 'execute').mockResolvedValue(tasks);
    ...
    it('should throw NotFoundException when no tasks are found with the specified title', async () => {
      ...
      await expect(controller.findByTitle(title)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('findByTag', () => {
    it('should return an array of tasks with the specified tag', async () => {
      ...
      const tasks = [
        new Task({ id: '1', title: 'Task 1', description: 'Task 1 description', dateTime: now, duration: 10, tags: [tag] }),
        new Task({ id: '2', title: 'Task 2', description: 'Task 2 description', dateTime: now, duration: 10, tags: [tag] })
      ];
    ...
    it('should throw NotFoundException when tag is not found', async () => {
      ...
      await expect(controller.findByTag(tagId)).rejects.toThrow(NotFoundException);
    });
  ...
  describe('update', () => {
    it('should update an existing task', async () => {
      ...
      jest.spyOn(findTaskByIdUseCase, 'execute').mockResolvedValue(existingTask);
      jest.spyOn(updateTaskUseCase, 'execute').mockResolvedValue(undefined);

      await expect(controller.update(taskId, updateTaskDto)).resolves.not.toThrow();
    });
